...
function GetSitecollections {
    ...
    $adminUrl = "https://$tenantBase.sharepoint.com"
    ...
    if ($inputFilePath)
    {
        ...
        {
            $returnSitecollections += Get-PnPTenantSite -Identity $site
        }
    }
    else
    {
...
function ScanWebs {
    param (
     ...
     # Connect to the web
     Connect-PnPOnline -Url $web.Url -ClientId $clientId -Thumbprint $thumbprint tenantFullName $tenantFullName
     ...
     foreach ($list in $lists){
         try {
             ...
             if($null -ne $listReportEntry){          
                 $reportLine=[System.String]::Format('"{0}","{1}",{2}',$sitecollectionUrl,$web.Url,$report[$report.Count-1])
...
function ScanSiteCollection {
    ...
    try {
        ...
        $rootWeb = Get-PnPWeb
        $SiteCollectionReportLines = ScanWebs -ClientId $clientId -Thumbprint $thumbprint -Tenant $tenantFullName -webId $rootWeb.Id -sitecollectionUrl $url
        $webs = Get-PnPSubWeb -Recurse;
        foreach($web in $webs){
            try {
                $webReportLines += ScanWebs -ClientId $clientId -Thumbprint $thumbprint -Tenant $tenantFullName -webId $web.Id -sitecollectionUrl $url
            }
 catch {
